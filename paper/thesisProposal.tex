...
% ------------------------------------------------------------------------------
\begin{abstract}
Finite element methods with global sparse matrices are not adequate for modeling exascale solid mechanics problems.
We are particularly interested in solving Neo-Hookean hyperelasticity at finite strain problems in the incompressible regime.
...
Additionally, we discuss traditional smoothers such as Jacobi and Chebyshev as well as a new domain decomposition based smoother using Balancing Domain Decomposition by Constraints with subdomain solvers using approximate separable inverses based on the Fast Diagonalization Method.
...
To verify the discretization, we performed a self convergence study for the strain energy.
For this study, we deformed a rectangular beam by performing a one radian axial shift on the right side of the beam while lowering the left side of the beam, as shown in Figure \ref{fig:deformation}.

\begin{figure}
\begin{subfigure}{.495\textwidth}
\includegraphics[width=.99\linewidth]{../img/deformBefore}
\caption{Beam Before Deformation}
\end{subfigure}
\begin{subfigure}{.495\textwidth}
\includegraphics[width=.99\linewidth]{../img/deformAfter}
\caption{Beam After Deformation}
\end{subfigure}
\caption{Deformation}
\label{fig:deformation}
\end{figure}

The results of this study can be seen in Figure \ref{fig:convergence}.
On the right, we fix the number of elements while increasing the polynomial degree of the basis.
On the left, we fix the polynomial degree of the basis while doubling the number of elements in each direction.
The degree 3 and degree 4 cases are not converging as rapidly as expected; however it is unclear if the convergence is a problem with the study or the implementation. 


\begin{figure}
\begin{subfigure}{.495\textwidth}
\includegraphics[width=.99\linewidth]{../img/convergeHRefine}
\caption{h Refinement}
\end{subfigure}
\begin{subfigure}{.495\textwidth}
\includegraphics[width=.99\linewidth]{../img/convergePRefine}
\caption{p Refinement}
\end{subfigure}
\caption{Self Convergence Study}
...
Matrix-free formulations require iterative solvers for the Galerkin system of equations.
...
As a result, preconditioning methods for CG can be applied to more general Krylov methods, such as generalized minimal residual method (GMRES).
...
In a new publication, Davydow et al. are using $h$-multigrid for matrix-free finite elements in solid mechanics; however $p$-multigrid offers more flexibility with respect to meshes in comparison to $h$-multigrid as it does not require aggregation of multiple elements into larger elements.
...
We are collaborating on a paper implementing these techniques in the context of compressible Neo-Hookean hyperelasticity at finite strain.
...
It relatively easy to provide adequate estimates of the maximal eigenvalue and Chebyshev is not too sensitive to estimation of this parameter.

Currently, we use the Chekyshev semi-iterative method as a smoother in our compressible Neo-Hookean hyperelasticity solver.
...
\label{bddc}
\end{equation}
where $\mathcal{H}$ is the direct sum of local operators $\mathcal{H}^{\left( i \right)} = - \left( \mathbf{A}_{I I}^{\left( i \right)} \right)^{-1} \left( 
mathbf{A}_{\Gamma I}^{\left( i \right)} \right)^T$ that map the jump over subdomain interfaces $J_D$ to subdomain interiors by solving a local Dirichlet problem and giving zero for other values so
\begin{equation}
...
The operator $\widetilde{\mathbf{A}}^{-1}$ represents a substructure solver and the operator $\mathbf{A}_{I I}^{\left( i \right)}$ represents a subdomain solver.
This subdomain solver is often a direct method; however, Li and Widlun demonstrated the feasibility of inexact subdomain solvers in \cite{li2007use}.

% Subdomain Solvers ------------------------------------------------------------
\subsubsection{Subdomain Solvers}\label{subdomainsolvers}

We are investigating separable approximate inverses based on the Fast Diagonalization Method (FDM) as inexact subdomain solvers for non-separable problems.
...
Notice that if we treat $\boldsymbol{\mathcal{X}}$ as a basis, these inverses can be described and implemented matrix-free, as in \ref{galerkin_form}.
...
In these cases, splitting these PDE by fields and applying different preconditioners to each fields can yield an effective preconditioner.
...
BDDC is another attractive technique for smoothing multigrid methods, and FDM based matrix-free separable approximate inverses may provide suitable subdomain solvers for BDDC and other domain decomposition techniques with non-linear and non-separable problems on arbitrarily deformed elements.
