...
% ------------------------------------------------------------------------------
\begin{abstract}
Global sparse matrices are no longer a good representation of a high-order    finite element operator.
...
Under these hardware constraints, global sparse matrices are no longer a good representation of a high-order finite element operator.
Matrix-free operators offer superior performance, both with respect to FLOPs needed for evaluation and the memory transfer needed for a matrix-vector product.
However, matrix-free methods require iterative solvers, which are sensitive to the condition number of the operator.

In Section 2, we discuss the specific hardware limitations that constrain the performance of finite element codes in HPC.
...
Two key performance metrics for high performance computing hardware are FLOPs and memory and network bandwidth.
FLOPs is the more well known of these two metrics; the Top 500 \cite{meuertop500} list tracks the 500 supercomputers with the highest peak  FLOPs, as measured by High-Performance Linpack (HPL) \cite{petitethpl}.
HPL measures the performance when solving random dense linear systems in double precision via LU factorization.
...
These 
The disparity between the FLOPs achieved in benchmarks such as HPGMG and HPCG and the peak FLOPs measured by HPL is partially explained by the difference between FLOPs and memory and network bandwidth.
...
\begin{figure}
\includegraphics[width=\linewidth]{img/mcclapinsc16plot}
\caption{30 year system balance history}
\label{fig:mccalpinsc16}
\end{figure}

As seen in Figure \ref{fig:mccalpinsc16}, from 2013 to 2016, FLOPs have improved twice as much as memory and network bandwidth.
...
Matrix-free representations of high-order finite element operators provide a way to reduce total FLOPs required to execute matrix-vector products and significantly reduce the memory transfer required.
...
In this section we develop notation to describe high-order finite elements on unstructured meshes implemented in a matrix-free fashion.
...
Element integration weights therefore become $\mathbf{W}^e = W \Lambda \left( \lvert J^e \left( q \right) \rvert \right)$.
...
Neumann or Robin boundary conditions are represented by adding boundary integral terms with basis and element restriction operations.
Boundary integrals internal to the domain $\Omega$, such as face integrals in Discontinuous Galerkin methods, can also be represented using additional terms with appropriate bases and element restrictions.


% -- Linearization -------------------------------------------------------------
\subsection{Linearization}

When the PDE \ref{weak_form} is linear, the pointwise functions $f_0 \left( \right)$ and $f_1 \left( \right)$ are also linear and Krylov subspace methods can be used to solve the Galerkin system of equations \ref{galerkin_form}.
When the PDE is non-linear, the Jacobian of the residual evaluator given in \ref{galerkin_form} ran be represented in a similar fashion, based upon the weak form
...
If these pointwise functions are not available analytically, they can be computed via algorithmic differentiation or finite differencing.
With these pointwise functions, Jacobian-free Newton-Krylov methods can be used to solve non-linear PDEs.
Jacobian-free Newton-Krylov methods were summarzied, with preconditioning strategies, by Knoll and Keyes in \cite{knoll2004jacobian}.


% -- Performance ---------------------------------------------------------------
\subsection{Performance}

To demonstrate the performance benefits of high-order finite elements implemented in a matrix free format, we explore the specific case of the Helmholz equations.
...
\label{helmholz_weak}
\end{equation}
with Galerkin system of equations

\begin{equation}
\sum_e \mathcal{E} \left[ \left( \mathbf{B} \right)^T \mathbf{W}^e \Lambda \left( \left( - k^2 \right) \mathbf{B} \mathcal{E}^e u \right) + \sum_{i = 0}^{d - 1} \left( \mathbf{D}_i^e \right)^T \Lambda \left( \lbrace \mathbf{D}_i^e \mathcal{E}^e u \rbrace_{i = 0}^{d - 1} \right) \right] = 0
\label{helmholz_galerkin}
\end{equation}
The total operations and matrix entries required to apply the matrix-vector product for the operator representing the Galerkin system of equations \ref{helmholz_galerkin} depends upon a specific partitioning of the domain $\Omega$ into elements.
For simplicity, we will compare the total floating point operations and matrix entries required to apply the operator representing \ref{helmholz_galerkin} for a single high-order element with arbitrary hexahedral geometry.
While fewer operations and matrix entries will be required on a domain decomposed into multiple elements due to the shared nodes on element boundaries, this comparison will adequately illustrate the relative merits of matrix-free implementations.

With a polynomial basis of degree $P - 1$, a 3D hexahedral element has $P^3$ nodes.
Therefore, the assembled matrix representing the Galerkin system of equations \ref{helmholz_galerkin} has $P^6$ entries.
...
With a $P$ point quadrature rule, the 1D basis operators, $\mathbf{B}$ and $\mathbf{D}$, have $2 P^2$ entries, the inverses of the Jacobians of the coordinate mappings requires $d^2 P^3$ entries, and the element quadrature weights require $P^3$ entries.
Including the spatial frequency, $\left( d^2 + 1 \right) P^3 + 2 P^2 + 1$ entries are required.
...
The analysis is similar for over-integration or under-integration.
If the LGL points are also used for then quadrature, then $2 P^3$ floating point operations are required to apply the basis operators, as $\mathbf{B} = I$.

\begin{figure}
\includegraphics[width=\linewidth]{img/assembledVsMatrixFree}
\caption{Performance per DoF}
\label{fig:assembledvsmatrixfree}
\end{figure}

As shown in \ref{fig:assembledvsmatrixfree}, the number of values required per node to represent the Galerkin system of equations \ref{helmholz_galerkin} in a matrix-free fashion is constant with respect to polynomial order, while the number of values required for the assembled matrix representing the operator grows exponentially.
...
We are primarily interested in Krylov subspace methods such as Conjugate Gradients, first developed by Hestens and Stiefel \cite{hestenes1952methods}, and related methods by Lanczos \cite{lanczos1950iteration} and \cite{lanczos1952solution}.
...
This system may come from the Galerkin system of equations of our PDE of interest or the Jacobian of our PDE of interest.
...
\label{jacobi}
\end{equation}
Computation of the full assembled linear operator for preconditioning defeats the benefits of matrix-free methods, but the true operator diagonal can be efficiently computed.
...
\label{jacobi_diag}
\end{equation}
The more diagonally dominant the linear operator $A$ is, the better $M_L^{-1}$ approximates the true inverse $A^{-1}$.
Point block diagonal assembly offers a middle ground between diagonal assembly and operator assembly for operators that are insufficiently diagonally dominant for Jacobi preconditioning based upon the true diagonal to be an effective preconditioner.
...
\label{chebyshev}
\end{equation}
where $\omega_k = 2 \frac{2 - \beta - \alpha}{\beta - \alpha} \frac{c_{k - 1} \left( \mu \right)}{c_k \left( \mu \right)}$, $\gamma = 2 \ \left( 2 - \alpha - \beta \right)$, $\hat{M} r^{k - 1} = b - A y^{k - 1}$, and $c_i$ are the Chekyshev polynomials with $\mu = 1 + \frac{1 - \beta}{\beta - \alpha}$.
...
Efficient Jacobi and Checbyshev smoothers are important as smoothers for multigrid preconditioners.
...
$p$-type multigrid, developed by Ronquist and Patera \cite{ronquist1987spectral}, is a natural choice for high-order finite elements on an unstructured mesh and can be with operators represented as in \ref{galerkin_form}.
Multigrid can be used as an independent solver, but we investigate the use of multigrid as a preconditioner for an Krylov method.
There has been work by Heys, Manteuffel, McCormick, and Olson on algebraic multigrid for high order finite elements \cite{heys2005algebraic}; however algebraic multigrid requires assembly of the finite element operator, which defeats the benefits of matrix-free implementation.
There are examples of using $h$-multigrid, such as \cite{davydov2019matrix}; however $p$-multigrid offers more flexibility with respect to meshes in comparison to $h$-multigrid.

For $p$-type multigrid with nodal bases, the prolongation operator interpolates to a higher order basis nodes and is defined by
...
As described by May, Sanan, Rupp, Knepley, and Smith in \cite{may2016extreme}, with these components we can implement $p$-type multigrid

\begin{algorithm}
\caption{$p$-type multigrid}
\label{multigrid}
\begin{algorithmic}[1]
\State Compute $x^k$
...
With even modest order finite elements, such as $P = 4$, $p$-type multigrid can substantially reduce the size of the global solution vector, which makes assembly of the finite element operator on the coarse grid tractable so that direct solvers such as Algebraic Multigrid can be used to solve the coarse problem.

While the theoretical foundation for $p$-type multigrid implemented in a matrix-free fashion on unstructured meshes has existed for quite some time, there do not appear to be many examples of these techniques being combined to practical problems.
...
We can partition the degrees of freedoms into two groups, those on the interface $\Gamma$ and those in the interior $I$.
...
We are interested in investigating separable approximate inverses based on the Fast Diagonalization Method (FDM) that approximate inverses to non-separable problems.
...
\end{equation}
where $\mathbf{M} = \mathbf{B}^T \mathbf{W} \mathbf{B}$ and $\mathbf{K}_i = \mathbf{D}_i^T \mathbf{W} \mathbf{D}_i$.
In this example, we neglect the terms arising from the coordinate mapping as they can result in a non-separable operator.

These operators $\mathbf{M}$ and $\mathbf{K}$ can be written in terms of the 1D operators, $M = B^T W B$ and $K = D^T W D$, as $\mathbf{M} = M \otimes M \otimes M$ and $\mathbf{D}_0 = D \otimes M \otimes M$.
...
\begin{equation}
A = \boldsymbol{\mathcal{X}} \left( \sum_{i = 0}^{d - 1} \mathbf{L} - k^2 I_3 \right) \boldsymbol{\mathcal{X}}^T
\label{helmholz_diag}
\end{equation}
with inverse

\begin{equation}
A^{-1} = \boldsymbol{\mathcal{X}}^T \left( \sum_{i = 0}^{d - 1} \mathbf{L}_i - k^2 I_3 \right)^{-1} \boldsymbol{\mathcal{X}}
\label{helmholz_inv}
\end{equation}
where $\boldsymbol{\mathcal{X}} = \mathcal{X} \otimes \mathcal{X} \otimes \mathcal{X}$, $\mathbf{L}_0 = L \otimes I \otimes I$.
...
For PDEs with arbitrarily deformed subdomains and non-linear pointwise functions $f_0$ and $f_1$, early experiments indicate that a suitable approximate inverse might be constructed by correctly selecting pesudoeigenvalues to use with pseudoeigenvectors from the mass and stiffness matrices given above.
...
These preconditioners might not be appropriate for more complex PDEs.
In these cases, splitting these PDE by fields and applying different preconditioners to different fields can yield results.
...
This solver will split the displacement and discontinuous pressure fields, applying $p$-type multigrid as a preconditioner to the displacement fields and block Jacobi as a preconditioner to the discontinuous pressure field.
...
We discussed the performance benefits of high-order finite elements implemented in a matrix-free fashion.
...
$p$-type multigrid with matrix-free prolongation and restriction operators is a natural fit for high-order matrix-free finite elements and has proven effective in solid mechanics problems.
BDDC is another attractive technique that can be implemented in a matrix-free fashion, and FDM based matrix-free separable approximate inverses may provide suitable subdomain solvers for BDDC and other domain decomposition techniques.
