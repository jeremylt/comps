...
We currently consider two formulations used in solid mechanics applications: Neo-Hookean hyperelasticity at small strain and Neo-Hookean hyperelasticity at finite strain.
We are primarily interested Neo-Hookean hyperelasticity at finite strain in the incompressible regime.
...
In this formulation, we solve for displacement $u \left( X \right)$ in the reference frame $X$.
...
Different constitutive models can define $\boldsymbol{S}$.

We therefore have the weak form
\begin{equation}
...
\begin{equation}
\int_{\Omega}{\nabla_X \boldsymbol{v} : d \boldsymbol{P}}
\label{finite_weak_linear}
\end{equation}
were $d \boldsymbol{P} = d \boldsymbol{F} \boldsymbol{S} + \boldsymbol{F} d \boldsymbol{S}$ and $d \boldsymbol{S}$, as above, depends upon the exact constitutive model chosen to determine the second Piola-Kirchhoff stress tensor.
...
As with sparse matries, matrix-free formulations require iterative solvers for solve the Galerkin system of equations.
We are primarily interested in Krylov subspace methods such as Conjugate Gradient (CG), first developed by Hestens and Stiefel \cite{hestenes1952methods}, and related methods by Lanczos \cite{lanczos1950iteration} and \cite{lanczos1952solution}.
...
Note that preconditioning CG is more restrictive than other Krylov methods since the CG algorithm requires preconditioners to preserve the symmetric positive definite property of the operator representing the Galerkin system.
Thus, preconditioning strategies for Conjugate Gradient are more restrictive and can be applied to more general Krylov methods, such as generalized minimal residual method (GMRES).
...
This solver will split the displacement and discontinuous pressure fields, applying $p$-multigrid to the displacement fields and block Jacobi to the discontinuous pressure field as part of a block preconditioner.
...
While Jacobi and Chebyshev semi-iterative preconditioning is not new, these techniques are important as smoothers for multigrid methods, and there is work to be done on providing efficient operator diagonal or point block diagonal assembly.
