...
High-order finite element methods offer advantages over low-order finite elements; $hp$ finite elements offer high arrucary and exponential convergence \cite{demkowicz1989toward}, \cite{oden1989toward}, \cite{rachowicz1989toward}.
...
Matix-free implementation of high-order finite elements can provide the benefits of high-order methods without relying upon matrix sparsity for efficient implementation.
...
% I don't quite understand why HPL does so well. Is it just that HPL measures FLOPs whereas HPGMG and HPCG are measuring "useful" FLOPs towards a more realistic objective?
...
\end{figure}
% I think this plot could be cleaned up for use in a paper. I'm not sure how 'Kosher' it is to recreate someone else's plot to make it match the style of plots in your paper. Is there a stadard practice here?
...
\end{figure}
% I think perhaps an additional plot next to this one showing the balance of FLOPs to entries would show why this is beneficial. Also, is entries the word I want? It feels awkward but I don't have a better word.
...
\label{chebyshev}
\end{equation}
% I haven't quite figured out the best way to write an iterative technique like this as a preconditioner M. I get that we are effectively using A^{-1} \approx M^{-1} here, but it seems difficult to write M = .... in this case.
...
% Should I try to mention why? I only have 5-10 pages to work with (technically), but I feel like this Section 4 is providing really brief overviews of the preconditioners I'm interested in implementing well in libCEED.
...
Ronquist and Patera declared $p$-type multigrid \textit{sensibly independent} of number of elements and polynomial degree of the element bases.
...
With these components we can implement $p$-type multigrid, using the algorithm described by May, Sanan, Rupp, Knepley, and Smith in \cite{may2016extreme}
...
With even modest order finite elements, such as degree $4$, $p$-type multigrid can substantially reduce the size of the global solution vector by a factor of $P^3 / 8$, which makes assembly of the finite element operator on the coarse grid tractable so that direct solvers such as Algebraic Multigrid can be used to solve the coarse problem.

While the theoretical foundation for $p$-type multigrid implemented in a matrix-free fashion on unstructured meshes has existed for quite some time, there do not appear to be many examples of these techniques being combined for practical problems.
...
% I feel like this is my best case to the committee that I have been doing valuable mathematical work and will be able to continue to do so for my thesis. But I'm pretty surprised that I didn't see any p-multigrid papers for solid mechanics, especially on unstructured meshes and especially with AMG as a coarse solve. I suppose I don't have a good senes of how novel this combination is?
...
In BDDC, the coarse problem is solved on a reduced set of shared DoFs between subdomains, using element corners and edges in 3D.
...
% I will be the first to admit that my understanding of BDDC needs some refinement.
...
% I feel like this will be my biggest mathematical piece of my thesis. Is it worth mentioning that I'm focused on BDDC but this effort to develop separable approximate inverses can be used for a wider range of domain decomposition problems? It seems like overlapping techniques would require a lot of recomputation for the large overlapping elements that would make computation of the pseudoeigenvalues possibly overly expensive in that case (q^3 vs (q+4)^3).
