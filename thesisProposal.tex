...
Under these hardware constraints, global sparse matrices are no longer good representation of a high-order finite element operators.
Matrix-free operators offer superior performance, both with respect to FLOPs needed for evaluation and the memory transfer needed for a matrix-vector product.
...
\label{jacobi}
\end{equation}
For matrix-free implementations, these values are not directly avaliable, as computation of the full assembled linear operator for preconditioning defeats the benefits of matrix-free methods; however, the true operator diagonal can be efficiently computed.
...
These preconditioners are appropriate for all PDEs.
